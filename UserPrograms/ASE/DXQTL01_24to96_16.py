...
for col in [2,3,4,5]:
	for row in [0,1,2,3]:
		...
		position(row,col)
		mix(320,adjusted_depth+2,100,5)

		# from DW24 to empty DW96W at c6
		position(row,col)
		aspirate(320, depth=adjusted_depth+2,speed=75, mix=0)
		position(col-2,6,position = OffsetDict[row])
		dispense(320, depth=adjusted_depth+1, speed=50)
		position(row,col)
		mix(320,adjusted_depth+3,100,2)
		position(row,col)
		aspirate(320, depth=adjusted_depth + 3,speed=75, mix=0)
