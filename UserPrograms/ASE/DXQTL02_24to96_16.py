...
for col in [2,4,5]:
	for row in [3]:
		...
		position(row,col)
		mix(320,adjusted_depth_row+6-(col-2),100,5)

		# from DW24 to empty DW96W at c6
		position(row,col)
		aspirate(320, depth=adjusted_depth_row+6-(col-2),speed=75, mix=0)
		position(col-2,6,position = OffsetDict[row])
		dispense(320, depth=adjusted_depth_column-2-(col - 2), speed=50)
		position(row,col)
		mix(320,adjusted_depth_row+6-(col-2),100,2)
		position(row,col)
		aspirate(320, depth=adjusted_depth_row + 6-(col-2),speed=75, mix=0)
		position(col-2,6,position = OffsetDict[row])
		dispense(320, depth=adjusted_depth_column-2-(col - 2), speed=50)
		position(row,col)
		mix(320,adjusted_depth_row+7-(col-2),100,2)
		aspirate(320, depth=adjusted_depth_row + 7-(col-2),speed=75, mix=0)
		position(col-2,6,position = OffsetDict[row])
		dispense(320, depth=adjusted_depth_column-2-(col - 2), speed=50)
